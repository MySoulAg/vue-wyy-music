      ...
      <van-swipe ref="swipeRef" :loop="false" initial-swipe="1" indicator-color="#fff">
        ...
        <van-swipe-item>
          <div class="nowList list-box">
            <h5>当前播放<span>({{getCurrentSongList.length}})</span></h5>
            <transition-group name="list" tag="ul">
              <li @click="playingSong(item.id)" :class="[getSongId==item.id?'active':'']" v-for="(item,index) in getCurrentSongList" :key="item.id">
